...
const releaseSchema=z.object({schemaVersion:z.literal(1),repository:z.string().regex(/^[A-Za-z0-9][A-Za-z0-9._-]*\/[A-Za-z0-9][A-Za-z0-9._-]*$/).nullable(),revision:z.string().regex(/^[a-f0-9]{40}$/).nullable(),status:z.enum(['draft','validated']).default('validated'),vessels:z.array(yachtVesselSchema),manifests:z.array(z.object({id:z.string(),path:assetPathSchema,sha256:z.string().regex(/^[a-f0-9]{64}$/)})).default([]),preview:z.boolean().optional()});
